...
;;;; Added changes: 8/9/2008
;;;; Added AND and OR
;;;; REQUIRE now returns okay
...
;; AND and OR, for doing logic puzzles more flexibly...

(define (and? exp) (tagged-list? exp 'and))
(define (or? exp) (tagged-list? exp 'or))


;; simplified AND, that doesn't return the value from
;; evaluating the last expression
(define (analyze-and exp)
  (define (combine-compiled-clauses proc1 proc2)
    (lambda (env succeed fail)
      (proc1 env
	     (lambda (val fail2)
	       (if (true? val)
		   ...
		   (succeed false fail2)))
	     fail)))
  (let ((clause-procs (map analyze (cdr exp))))
    (accumulate combine-compiled-clauses
		(lambda (env succeed fail) (succeed true fail))
		clause-procs)))

;; simplified OR, even though it's easy to keep the
;; same behavior. This is to be consistent with
;; the behavior of AND above.
