...
(define *traced-method-table* (make-table))
...
(define (ask object message . args)
  (let ((method (object message)))
    (if (method? method)
        (let ((trace-info (lookup *traced-method-table* method)))
             (if trace-info
                 (begin (format *err-port* "~A -> ~A method ~A with "
                                (indent)
                                (car trace-info) (cadr trace-info))
                        (show-args (cddr trace-info) args)
                 ))
             (let ((result (apply method args)))
                  (if trace-info
                      (begin (format *err-port*
                                     "~A <- ~A method ~A returns ~S\n"
                                     (indent)
                                     (car trace-info) (cadr trace-info)
                                     (result)))))
        )
	(error "Instance of class " (cadr method)
               " does not respond to message: " message))))
		 ...
		 ((eq? class-message 'instantiate)
		  (lambda ,(instantiation-vars form)
			...
			(cond
			 ...
			 ,(else-clause form) ))
		      dispatch )))
		 (else (error "Class " ',(class-name form)
                              " does not respond to message: " class-message))
                              )))))))
...
(define (usual-clause form)
  (let ((parent-clause (find-a-clause 'parent form)))
	...
	`((eq? message 'send-usual-to-parent)
	  (error "Can't use USUAL in class " ',(class-name form)
	...
	`((eq? message 'send-usual-to-parent)
	  (lambda (message . args)
	    (let ((method (get-method ',(class-name form)
	      ...
	      (if (method? method)
		  (apply method args)
		  (error "Class " ',(class-name form)
                         "has no USUAL method for '" message)
                         )))))))
